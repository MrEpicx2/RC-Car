...
int main(void) {
    ...
    TCA0.SINGLE.CTRLA = (TCA_SINGLE_CLKSEL_DIV256_gc) | (TCA_SINGLE_ENABLE_bm);     //Divide the 8Mhz by 256 and enable perferal
    ...
    while (1) {
        ...
        servo_position = 31 + ((result * 32UL) / 4095);
        TCA0.SINGLE.CMP0 = servo_position;
        _delay_ms(10);
    }
}
