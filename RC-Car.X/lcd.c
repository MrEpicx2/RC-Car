...
void lcd_command(uint8_t *cmd, uint8_t size) {
    i2c_start(LCD_I2C_ADR << 1); // Start transmission
    i2c_byte(0x00); // Command mode
...
void lcd_init(void) {
    _delay_ms(100);  // Ensure power-up delay

    for (uint8_t i = 0; i < sizeof(init_sequence); i++) {
        i2c_start(LCD_I2C_ADR << 1); // Start for each command
        i2c_byte(0x00); // Command mode
        i2c_byte(init_sequence[i]); // Send command
        i2c_stop();
        _delay_ms(1); // Small delay to allow processing
    }
    
    // **Manually turn display ON at the end**
    i2c_start(LCD_I2C_ADR << 1);
    i2c_byte(0x00);
    i2c_byte(0xAF);  // **Force Display ON**
    i2c_stop();
}
...
void lcd_goto_xpix_y(uint8_t x, uint8_t y){
    ...
    uint8_t commandSequence[] = {
        0xB0 + y,      // Set Page Start Address
        0x21, x, DISPLAY_WIDTH - 1, // Set Column Address (start x to full width)
    };
#elif defined SH1106
...
void lcd_clrscr(void) {
    for (uint8_t page = 0; page < (DISPLAY_HEIGHT / 8); page++) { // Iterate over rows (pages)
        lcd_gotoxy(0, page); // Set cursor to start of row
        
        // Send 128 bytes of 0x00 to clear this row
        uint8_t clearLine[DISPLAY_WIDTH]; 
        memset(clearLine, 0x00, DISPLAY_WIDTH); // Fill with zeros
        lcd_data(clearLine, DISPLAY_WIDTH); // Send to screen
    }
    lcd_home();
}
